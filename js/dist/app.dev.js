...
var btn = document.getElementById('add-btn'); // =================================ADD ITEM TO TASK LIST============================

btn.addEventListener('click', function (e) {
  e.preventDefault(); // var input tdg

  var addInput = document.getElementById('add-input'); //Avoid submitting empty label
  ...
  if (addInput.value !== '') {
    ...
    var li = document.createElement('li'),
        ...
        input = document.createElement('input'); //styling the icon

    firstIcon.style.padding = '10px 10px';
    firstIcon.style.color = "red"; //adding css clsassName attribute

    secondIcon.className = "fa-sharp fa-solid fa-pen-to-square";
    firstIcon.className = "fa-solid fa-xmark";
    input.className = '"edit-note"';
    textParagraph.className = "text"; //setAttribute for addInput

    input.setAttribute('type', 'text');
    textParagraph.textContent = addInput.value; //assign the list element to its children

    iconParagraph.appendChild(firstIcon);
    iconParagraph.appendChild(secondIcon); //  secondIcon.appendChild(svgIcon);

    li.appendChild(textParagraph);
    li.appendChild(iconParagraph);
    li.appendChild(input);
    ul.appendChild(li); //clear text input clicking on add button
...
ul.addEventListener('click', function (e) {
  ...
  if (e.target.classList[2] === 'fa-pen-to-square') {
    ...
    var parentParagraph = e.target.parentNode; //  we need the parentParagraph when click event occur

    parentParagraph.style.display = 'none';
    var note = parentParagraph.previousElementSibling;
    var input = parentParagraph.nextElementSibling; // to appear input element same time of event 

    input.style.display = 'block'; // display input text in block
    //taking note for input

    input.value = note.textContent; ///Event Listeners for handing input value using kepress event 
    ...
    input.addEventListener('keypress', function (e) {
      ...
      if (e.keyCode === 13) {
        if (input.value !== '') {
          ...
          note.textContent = input.value; // we need to access the icon delete

          parentParagraph.style.display = 'block';
          input.style.display = 'none'; //  weed to control the ul from submiting when blank
        } else {
...
searchItems.addEventListener('keyup', function (e) {
  ...
  var searchChar = searchItems.value.toUpperCase(); //  console.log(searchChar)
  //accessing the list items by tag name.

  var notes = ul.getElementsByTagName('li'); //in order to compare the items we neeed to loop through the items.

  Array.from(notes).forEach(function (note) {
    // let grab the content of list items
    var paragraphText = note.firstElementChild.textContent; // transform the paragraphText toUpperCase
